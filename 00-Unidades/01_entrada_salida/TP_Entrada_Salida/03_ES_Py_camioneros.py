...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ...
        operacion_kg = toneladas *1000
        operacion_final= math.ceil(operacion_kg / 3500)
        mensaje = f"La cantidad de camiones a ultilizar es de {operacion_final}"
        alert("", message=mensaje)
        #El match ceil sirve para redondear para arriba una operacion ya sea que la misma este mas aproximada a su anterior numero

    def btn_tiempo_llegada_on_click(self):
        Kilometros = float(self.txt_kilometros.get())
        Kilometros_a_horas= round(Kilometros/ 90)
        mensaje= f"El tiempo en que el camion llegara a destino es en {Kilometros_a_horas} hora/s "
        alert("", message=mensaje)
        #El roun sirve para redondear una operacion llevandola a su numero entero mas cercano sin coma
